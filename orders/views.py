...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['name', 'email', 'phone', 'id', 'order_number']
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        if period == 'today':
            ...
            seven_days_ago = timezone.localtime() - timezone.timedelta(days=7)
            order_qs = order_qs.filter(created_at__gte=seven_days_ago)
            order_item_qs = order_item_qs.filter(order__created_at__gte=seven_days_ago)
        elif period == '1_month' or period == 'this_month':
            thirty_days_ago = timezone.localtime() - timezone.timedelta(days=30)
            order_qs = order_qs.filter(created_at__gte=thirty_days_ago)
            order_item_qs = order_item_qs.filter(order__created_at__gte=thirty_days_ago)
        else:
